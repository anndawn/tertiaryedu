var chart2=echarts.init(document.getElementById('g2'), null, {renderer: 'svg'});
    var options;
    let datachart2=[
 {
   "LOCATION": "BEN",
 ...
 newdatachart2=datachart2.map(function(d) {
  return {
    popu: d["Value"],
    time:d["Time"],
    country: d["Country"]
...
let archart2=[];
 var newdatachart2 = d3.nest()
                  .key(function(d) { return d.country; })
                  .entries(newdatachart2);
                  console.log(newdatachart2);
let keyschart2=[];
for (var i = 0; i < newdatachart2.length; i++) {
  keyschart2.push(newdatachart2[i]["key"])
}
console.log(keys);
let objchart2=[];
for(let i of newdatachart2){let detail=i["values"] ;
let arr=[];
let arr1=[];
for(let k of detail){console.log(k);
  let per="";
  if (k["popu"]) {
    per=k["popu"].toFixed(2)
  }else {
    per=k["popu"]
  }
  arr.push(per);
